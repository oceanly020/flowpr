				...
				if path == False:
					...
					for s in path: # update path
						if s == dpid_dst:
							...
							msg.match.dl_vlan = packetflow.dl_vlan
							msg.match.dl_vlan_pcp = packetflow.dl_vlan_pcp
							msg.match.dl_src = packetflow.dl_src # Wildcard dst IP
							msg.match.dl_dst = packetflow.dl_dst
							msg.match.nw_proto = packetflow.nw_proto
							msg.match.dl_type = packetflow.dl_type
							msg.match.nw_tos = packetflow.nw_tos
							msg.match.tp_src = packetflow.tp_src
							msg.match.tp_dst = packetflow.tp_dst
...
class packet_flow(object):
	...
	def flow_from_packet(self, packet): # head of match of packet_in
		...
		self.dl_src = packet.src
		self.dl_dst = packet.dst
		self.dl_type = packet.type
		p = packet.next
		if isinstance(p, vlan):
		  self.dl_type = p.eth_type
		  self.dl_vlan = p.id
		  self.dl_vlan_pcp = p.pcp
		  p = p.next
		else:
		...
		if isinstance(p, ipv4):
			self.nw_src = p.srcip
			self.nw_dst = p.dstip
			self.nw_proto = p.protocol
			self.nw_tos = p.tos
			p = p.next

			if isinstance(p, udp) or isinstance(p, tcp):
				self.tp_src = p.srcport
				self.tp_dst = p.dstport
			elif isinstance(p, icmp):
				self.tp_src = p.type
				self.tp_dst = p.code
		elif isinstance(p, arp):
			if p.opcode <= 255:
				self.nw_proto = p.opcode
				self.nw_src = p.protosrc
				self.nw_dst = p.protodst
				...
				for f in lvl_element[i]:
					...
					if contain == True:
						for f2 in self.nodes_up:
							if f in self.nodes_up.related_flow:
								continue
							else:
...
class flow (object):
	...
	def location_find(self):
		...
		for x in range(1,self.level - 1): # find the related flow above this level
			i = self.level - x
			for f in lvl_element[i]:
				contain = True
				for m in match_data:
					...
					for f2 in self.nodes_up:
						if f in self.exception: # meet the except flow
							...
							if f in self.nodes_up.related_flow:
								...
								self.nodes_up.append(f)
								f.nodes_down.append(self)
		if len(self.nodes_up) == 0:
			self.nodes_up.append(flow_root)
			flow_root.nodes_down.append(self)
						...
						for f2 in fl.nodes_up:
							if f in fl.exception: # meet the except flow
								...
								if f in fl.nodes_up.related_flow:
									...
									f.nodes_down.append(fl)
			if len(fl.nodes_up) == 0:
				fl.nodes_up.append(flow_root)
				flow_root.nodes_down.append(fl)
			fl.calc_flow_graph()
		for x in range(self.level + 1,len(match_data) + 1): # update flow under this level
			for f in lvl_element[x]:
		...
		for s1 in switchlist:
			for s2 in switchlist:
				if s2 not in allow_set:
					self.actions_graph[switchlist.index(s1)][switchlist.index(s2)] = 60000
...
def launch():
	...
	#	_hold_down = True
	__init__()
	# flow1 = flow(dl_src = IPAddr("10.0.0.1"),
	# 			 dl_dst = IPAddr("10.0.0.2"))
	# flow1_path = [2,5,8,10,11]
	# flow1.attach_to_path(path = flow1_path)
